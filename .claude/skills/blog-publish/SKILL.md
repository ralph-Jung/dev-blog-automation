...
3. `.claude/agents/seo-content.md` 기준을 적용하여 SEO 최적화:
   - 헤딩 구조: H2→H3 논리적 계층 유지 (레벨 건너뛰기 금지)
   - 키워드: 제목과 첫 문단에 주요 키워드 포함, 본문 내 자연스러운 밀도(1-3%)
   - 분량: 블로그 포스트 최소 1,500단어 이상 확보
   - 가독성: 문단 2-4문장, 비교/나열은 표·리스트 활용
4. `.claude/agents/seo-geo.md` 기준을 적용하여 AI 검색 최적화:
   - 각 섹션 첫 40-60단어 내에 핵심 답변 배치
   - 질문형 소제목 활용 (예: "가상 메모리란?" → AI가 인용하기 좋은 구조)
   - 자기완결형 답변 블록: 134-167단어 길이의 인용 가능한 문단 포함
   - 정의 문장은 "X는 ~이다" 패턴으로 명확하게 작성
